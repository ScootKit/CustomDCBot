...
module.exports.run = async function (client, msg, args) {
    ...
    if (!msg.member.roles.cache.find(r => moduleConfig['moderator-roles_level3'].includes(r.id) || moduleConfig['moderator-roles_level4'].includes(r.id))) return message.edit(moduleStrings['no_permissions'].split('%required_level%').join(3));
    let user;
    ...
    if (user.roles.cache.find(r => moduleConfig['moderator-roles_level3'].includes(r.id) || moduleConfig['moderator-roles_level4'].includes(r.id))) return message.edit(moduleStrings['this_is_a_mod']);
