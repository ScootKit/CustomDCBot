...
module.exports.run = async function (interaction) {
    ...
    if (member.user.id === interaction.user.id) return interaction.reply({
        ephemeral: true,
        content: '⚠️️ ' + localize('duel', 'self-invite-not-possible', {r: `<@${((await interaction.guild.members.fetch({withPresences: true})).filter(u => u.presence && u.user.id !== interaction.user.id && !u.user.bot).random() || {user: {id: 'RickAstley'}}).user.id}>`})
    });
    ...
    a.on('collect', (i) => {
        if (!started) {
            if (i.user.id !== member.id) return i.reply({
                ephemeral: true,
                content: '⚠️️ ' + localize('duel', 'you-are-not-the-invited-one')
            });
        ...
        if (!i.customId.includes('invite')) {
            if (i.user.id !== interaction.user.id && i.user.id !== member.user.id) return i.reply({
                ephemeral: true,
                content: '⚠️️ ' + localize('duel', 'not-your-game')
            });
            ...
            if (action === 'reload') {
                if (bullets[i.user.id] === 5) return i.reply({
                    ephemeral: true,
                    content: '⚠️️ ' + localize('duel', 'bullets-full')
                });
                bullets[i.user.id]++;
            }
            if (action === 'gun') {
                if (bullets[i.user.id] === 0) return i.reply({
                    ephemeral: true,
                    content: '⚠️️ ' + localize('duel', 'no-bullets')
                });
