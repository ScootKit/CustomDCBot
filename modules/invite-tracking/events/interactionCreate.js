...
exports.run = async (client, interaction) => {
    ...
    if (interaction.customId.startsWith('uinv-rev')) {
        ...
        try {
            ...
            await interaction.editReply({
                content: '⚠️️ ' + localize('invite-tracking', 'invite-revoked-error', {
                    e,
                    c
                })
            });
    ...
    if (!interaction.member.permissions.has('MANAGE_GUILD')) return interaction.reply({
        ephemeral: true,
        content: '⚠️️ ' + localize('invite-tracking', 'missing-revoke-permissions')
    });
    ...
    if (!invite) return interaction.reply({
        ephemeral: true,
        content: '⚠️️ ' + localize('invite-tracking', 'invite-not-found')
    });
    ...
    invite.delete(localize('invite-tracking', 'invite-revoke-audit-log', {u: formatDiscordUserName(interaction.user)})).then(() => {
        ...
        interaction.reply({
            ephemeral: true,
            content: '⚠️️ ' + localize('invite-tracking', 'invite-revoked-error', {e, c: code})
        });
    });
};