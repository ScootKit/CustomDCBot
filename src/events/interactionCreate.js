...
module.exports.run = async (client, interaction) => {
    if (!client.botReadyAt) {
        ...
        return interaction.reply({
            content: '⚠️️ ' + localize('command', 'startup'),
            ephemeral: true
        });
    }
    if (client.guild.id !== interaction.guild.id) {
        if (interaction.isAutocomplete()) return interaction.respond({});
        return interaction.reply({
            content: '⚠️️ ' + localize('command', 'wrong-guild', {g: client.guild.name}),
            ephemeral: true
        });
    }
    ...
    if (!command) {
        ...
        else return interaction.reply({content: '⚠️️ ' + localize('command', 'not-found'), ephemeral: true});
    }
    if (command.module && !client.modules[command.module].enabled) return interaction.reply({
        ephemeral: true,
        content: '⚠️️ ' + localize('command', 'module-disabled', {m: command.module})
    });
    ...
    try {
        ...
        if (!command.subcommands) {
            ...
            return interaction.reply({
                content: '⚠️️ This command is not configured correctly and can not be executed, please contact the developer.',
                ephemeral: true
            });
        }
