...
module.exports.run = async function (client, msg) {
    ...
    for (const module in modules) {
        ...
        modules[module].forEach(d => {
            ...
            content = content + `\n\`${client.config.prefix}${c}\`: ${client.messageCommands.get(d).help.description}`;
        });
        embedFields.push({
    ...
    embedFields.filter(f => f.name === client.strings.helpembed.build_in).forEach(f => addSite(
        [
            f,
            {
                name: '\u200b',
